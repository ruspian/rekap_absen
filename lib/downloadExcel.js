...
// FUNGSI DOWNLOAD EXCEL BAKAT DAN MINAT SISWA
export const handleDownloadExcelBakatMinat = (bakatSiswa) => {
  // Mapping data sesuai field yang mau ditampilkan
  const cleanData = bakatSiswa.map((bakat, index) => ({
    No: index + 1,
    Nama: bakat.siswa.nama,
    Bakat: bakat.bakat || "-",
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Data Siswa");
  XLSX.writeFile(workbook, "Bakat-minat-siswa.xlsx");
};

// FUNGSI DOWNLOAD EXCEL PEMBINAAN WALI
export const handleDownloadExcelPembinaanWali = (initialData) => {
  // Mapping data sesuai field yang mau ditampilkan
  const cleanData = initialData.map((data, index) => ({
    No: index + 1,
    ...
    Arahan_Wali_Kelas: data.arahan,
    Tindak_Lanjut: data.kesepakatan || "-",
    TTD_Siswa: " ",
    TTD_Wali: " ",
    Keterangan: " ",
  }));
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Data Siswa");
  XLSX.writeFile(workbook, "Pembinaan-Wali.xlsx");
};

// FUNGSI DOWNLOAD EXCEL PEMBINAAN KASUS
export const handleDownloadExcelPembinaanKasus = (initialData) => {
  // Mapping data sesuai field yang mau ditampilkan
  const cleanData = initialData.map((data, index) => ({
    No: index + 1,
    Tanggal: formatTanggal(data.tanggal) || "-",
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Data Siswa");
  XLSX.writeFile(workbook, "Pembinaan-Kasus.xlsx");
};

// FUNGSI DOWNLOAD EXCEL INVENTARIS
export const handleDownloadExcelInventaris = (initialData, kelas) => {
  // Mapping data sesuai field yang mau ditampilkan
  const cleanData = initialData.map((data, index) => ({
    No: index + 1,
    No_Kode: "-",
    Nama: data.nama,
    Jumlah_Awal_Tahun: data.jumlah_awal,
    Jumlah_Akhir_Tahun: data.jumlah_akhir,
    Kondisi_Baik: data.baik,
    Kondisi_Rusak: data.rusak,
    Keterangan: " ",
  }));
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Data Siswa");
  XLSX.writeFile(workbook, "Inventaris_" + kelas + ".xlsx");
};
