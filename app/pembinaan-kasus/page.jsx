...
const PembinaanKasusPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [siswa, kelas, kepsek, pembinaanKasus] = await Promise.all([
        getSiswa(),
        getKelas(),
        getKepalaSekolah(),
        getPembinaanKasus(),
  ...
  const Icons = [
    ...
    {
      Icon: FileDown,
      onClick: () => handleDownloadExcelPembinaanKasus(dataPembinaanWali),
      className: "hover:bg-accent",
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="my-10 flex gap-2 items-center">
        <Target />
        <h2 className="text-2xl font-semibold">PEMBINAAN KASUS</h2>
      </div>
      ...
      <div ref={tableRef}>
        <TabelPembinaanKasus
          data={filteredData}
          kepsek={dataKepsek}
          pembinaanKasus={dataPembinaanKasus}
          onSuccess={fetchData}
        />
      </div>
...
export default PembinaanKasusPage;
